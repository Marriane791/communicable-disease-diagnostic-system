...
# Takes the response and converts it into a python object and insert it into mongodb
...
async def list_all_users(request:Request):
    users = []
    for user in await request.app.mongodb["users"].find().to_list(length=100):
        users.append(user)
    return users
...
async def update_user(id:str,request:Request,user_updated:UpdateUserModel = Body(...)):
    ...
    if len(user_updated) >= 1:
        new_details = await request.app.mongodb["users"].update_one(
            {"_id":id},{"$set":user_updated}
        )

        if new_details.modified_count == 1:
            if (
                final := await request.app.mongodb["users"].find_one({"_id":id})
            ) is not None:
                return final
...
async def delete_user(id:str, request:Request):
    revoke = await request.app.mongodb["users"].delete_one({"_id":id})
    

    if revoke.delete_count == 1:
